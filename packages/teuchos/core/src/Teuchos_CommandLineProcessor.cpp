  ...
  ) const
{
  ...
  if (check_for_help(argc, argv, errout)) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (handle_echo_or_pause(opt_name, argc, argv, errout)) continue;
    EParseCommandLineReturn r = handle_unrecognized_or_set_opt(i, argv, opt_name, opt_val_str, errout);
    if (r != PARSE_SUCCESSFUL) return r;
  }
  if (!validate_required_options(argc, argv, errout)) return PARSE_ERROR;
  apply_output_setup_to_default_stream();
...
CommandLineProcessor::EParseCommandLineReturn
CommandLineProcessor::handle_unrecognized_or_set_opt(
  int i, char* argv[], const std::string &opt_name, const std::string &opt_val_str, std::ostream *errout
  ) const
{
  const int procRank = GlobalMPISession::getRank();
  ...
  if (itr == options_list_.end()) {
    ...
    if (recogniseAllOptions())
      ...
      return PARSE_SUCCESSFUL; // ignore unrecognized when allowed
  }
...
bool CommandLineProcessor::check_for_help(int argc, char* argv[], std::ostream* errout) const
{
...
bool CommandLineProcessor::handle_echo_or_pause(const std::string &opt_name, int argc, char* argv[], std::ostream* errout) const
{
...
bool CommandLineProcessor::validate_required_options(int argc, char* argv[], std::ostream* errout) const
{
  for (options_list_t::const_iterator itr = options_list_.begin(); itr != options_list_.end(); ++itr) {
...
void CommandLineProcessor::apply_output_setup_to_default_stream() const
{

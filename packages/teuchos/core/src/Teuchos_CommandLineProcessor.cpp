  ...
  ) const
{
  ...
  // Pre-scan for --help
  EParseCommandLineReturn helpStatus = check_help_option(argc, argv, errout);
  if (helpStatus != PARSE_SUCCESSFUL) return helpStatus;

  // Process all options
  std::string opt_name, opt_val_str;
  for (int i = 1; i < argc; ++i) {
    const bool gov_return = get_opt_val(argv[i], &opt_name, &opt_val_str);
    if (!gov_return) {
      if (procRank == 0) print_bad_opt(i, argv, errout);
      if (recogniseAllOptions()) return PARSE_UNRECOGNIZED_OPTION; else continue;
    }

    // Built-in options like echo and pause
    if (handle_built_in_option(opt_name, argc, argv, procRank, errout))
      continue;

    // Regular table-driven options
    EParseCommandLineReturn setStatus = set_option_from_tables(
      i, opt_name, opt_val_str, argc, argv, procRank, errout);
    if (setStatus != PARSE_SUCCESSFUL) return setStatus;
  }

  // Validate required options
  EParseCommandLineReturn reqStatus = validate_required_options(argc, argv, errout);
  if (reqStatus != PARSE_SUCCESSFUL) return reqStatus;

  // Apply output setup options
  apply_output_setup_options();
...
CommandLineProcessor::EParseCommandLineReturn
CommandLineProcessor::check_help_option(
...
bool CommandLineProcessor::handle_built_in_option(
  ...
  if (opt_name == echo_cl_opt) {
    if (errout && procRank == 0) {
      *errout << "\nEchoing the command-line:\n\n";
      for (int j = 0; j < argc; ++j) *errout << argv[j] << " ";
      *errout << "\n\n";
    }
    return true;
  }
...
CommandLineProcessor::EParseCommandLineReturn
CommandLineProcessor::set_option_from_tables(
  int argv_i,
  const std::string &opt_name,
  ...
  int argc, char* argv[], int procRank, std::ostream *errout) const
{
  ...
  if (itr == options_list_.end()) {
    if (procRank == 0) print_bad_opt(argv_i, argv, errout);
    if (recogniseAllOptions()) return PARSE_UNRECOGNIZED_OPTION; else return PARSE_SUCCESSFUL;
  }
  opt_val_val_t &opt_val_val = (*itr).second;
  opt_val_val.was_read = true;
  switch (opt_val_val.opt_type) {
    case OPT_BOOL_TRUE:
      *(any_cast<bool*>(opt_val_val.opt_val)) = true; break;
    case OPT_BOOL_FALSE:
      *(any_cast<bool*>(opt_val_val.opt_val)) = false; break;
    case OPT_INT:
      *(any_cast<int*>(opt_val_val.opt_val)) = asSafe<int>(opt_val_str); break;
    case OPT_LONG_INT:
      *(any_cast<long int*>(opt_val_val.opt_val)) = asSafe<long int>(opt_val_str); break;
    case OPT_SIZE_T:
      *(any_cast<size_t*>(opt_val_val.opt_val)) = asSafe<size_t>(opt_val_str); break;
    case OPT_LONG_LONG_INT:
      *(any_cast<long long int*>(opt_val_val.opt_val)) = asSafe<long long int>(opt_val_str); break;
    case OPT_DOUBLE:
      *(any_cast<double*>(opt_val_val.opt_val)) = asSafe<double>(opt_val_str); break;
    case OPT_FLOAT:
      *(any_cast<float*>(opt_val_val.opt_val)) = asSafe<float>(opt_val_str); break;
    case OPT_STRING:
      *(any_cast<std::string*>(opt_val_val.opt_val)) = remove_quotes(opt_val_str); break;
    case OPT_ENUM_INT:
      if (!set_enum_value(argv_i, argv, opt_name, any_cast<int>(opt_val_val.opt_val),
...
CommandLineProcessor::EParseCommandLineReturn
CommandLineProcessor::validate_required_options(
  int argc, char* argv[], std::ostream *errout) const
{
...
void CommandLineProcessor::apply_output_setup_options() const
{

  ...
  ) const
{
  ...
  // Delegate help-checking to helper function
  EParseCommandLineReturn tmp_ret = parseCheckForHelpOpt(argc, argv, errout);
  if (tmp_ret != PARSE_SUCCESSFUL) return tmp_ret;
...
// Helper to check for help option before processing all options.
CommandLineProcessor::EParseCommandLineReturn
  ...
  ) const
{
  ...
  const std::string  help_opt = "help";
  const std::string  pause_opt = "pause-for-debugging";
  // We don't need procRank here as help only prints
  for( int i = 1; i < argc; ++i ) {
    bool gov_return = get_opt_val( argv[i], &opt_name, &opt_val_str );

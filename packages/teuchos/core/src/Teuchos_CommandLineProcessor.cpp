  ...
  ) const
{
  ...
  // Handle help early, before mutating any options
  if (contains_help_and_print(argc, argv, errout)) {
    return PARSE_HELP_PRINTED;
  }

  // Process all other options
  for (int i = 1; i < argc; ++i) {
    const EParseCommandLineReturn rc =
      process_single_arg(i, argc, argv, procRank, errout);
    if (rc != PARSE_SUCCESSFUL) return rc; // early return on error conditions
  }

  // Validate required options
  {
    const EParseCommandLineReturn rc =
      validate_required_options(argc, argv, errout);
    if (rc != PARSE_SUCCESSFUL) return rc;
  }
...
bool CommandLineProcessor::contains_help_and_print(
  int             argc,
  char*           argv[],
  std::ostream*   errout
  ) const
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (gov_return && opt_name == std::string("help")) {
...
CommandLineProcessor::EParseCommandLineReturn
CommandLineProcessor::process_single_arg(
  int             argv_i,
  int             argc,
  char*           argv[],
  int             procRank,
...
{
  std::string opt_name, opt_val_str;
  const bool gov_return = get_opt_val(argv[argv_i], &opt_name, &opt_val_str);
  if (!gov_return) {
    if (procRank == 0)
    ...
    if (recogniseAllOptions())
      ...
      return PARSE_SUCCESSFUL; // ignore and continue
  }

  // Built-in special options
  if (opt_name == std::string("echo-command-line")) {
    echo_command_line(argc, argv, errout, procRank);
    return PARSE_SUCCESSFUL;
  }
  if (opt_name == std::string("pause-for-debugging")) {
    pause_for_debugging(procRank);
    return PARSE_SUCCESSFUL;
  }

  // Lookup user-registered option
  ...
  // Set the option value according to type
...
void CommandLineProcessor::echo_command_line(
  int             argc,
  char*           argv[],
  std::ostream*   errout,
  int             procRank
...
void CommandLineProcessor::pause_for_debugging(
  int             procRank
  ) const
{
#ifndef _WIN32

  ...
  ) const
{
  ...
  EParseCommandLineReturn ret = handleHelpOption(argc, argv, errout);
  if(ret == PARSE_HELP_PRINTED)
    return PARSE_HELP_PRINTED;

  ret = processOptions(argc, argv, errout, procRank);
  if(ret != PARSE_SUCCESSFUL)
    return ret;

  ret = checkRequiredOptions(argv[0], errout);
  if(ret != PARSE_SUCCESSFUL)
    return ret;
...
/* Extracted helper functions for parse() */

EParseCommandLineReturn CommandLineProcessor::handleHelpOption(int argc, char* argv[], std::ostream* errout) const {
  std::string opt_name;
...
EParseCommandLineReturn CommandLineProcessor::processOptions(int argc, char* argv[], std::ostream* errout, int procRank) const {
  std::string opt_name;
  std::string opt_val_str;
...
EParseCommandLineReturn CommandLineProcessor::checkRequiredOptions(const char* programName, std::ostream* errout) const {
  for (options_list_t::const_iterator itr = options_list_.begin();
    ...
    if (opt_val_val.required && !opt_val_val.was_read) {
      ...
      if (errout)
        *errout << std::endl << programName << " : Error, the option --" << opt_val_name << " was required but was not set!" << std::endl;
      if (throwExceptions())
        TEUCHOS_TEST_FOR_EXCEPTION(true, ParseError, "Error, the option --" << opt_val_name << " was required but was not set!");
      return PARSE_ERROR;
    }
  }
  return PARSE_SUCCESSFUL;
...
void CommandLineProcessor::setupDefaultOutput() const {
